...
DA_BIN=/home/runner/work/Admini/Admini/backend/directadmin
TASKQ=/home/runner/work/Admini/Admini/backend/data/task.queue
DA_DATA_USERS=/home/runner/work/Admini/Admini/backend/data/users
	...
	for i in `cat ${DA_DATA_USERS}/$1/domains.list`; do
	{
		...
		if [ "${SYSTEM_USER_TO_VIRTUAL_PASSWD}" = "1" ] && [ -e "${NHOME}/Maildir" ] && ${DEFAULT_DOMAIN}; then
			...
			if [ -z "${MAIL_LOCATION}" ] || [ ! -d "${MAIL_LOCATION}" ]; then
				MAIL_FOLDER=`/home/runner/work/Admini/Admini/backend/directadmin c | grep '^mail_partition=' | cut -d= -f2`
                MAIL_LOCATION=${NHOME}
...
httpd_swap()
{
	...
	#/home/runner/work/Admini/Admini/backend/data/users/$1/httpd.conf
...
mysql_swap()
{
	...
	VERBOSE=$VERBOSE DBUSER="$MYSQL_USER" DBPASS="$MYSQL_PASS" DBHOST="$DBHOST" USERNAME="$1" NEWUSERNAME="$2" /usr/local/bin/php -c /home/runner/work/Admini/Admini/scripts/php_clean.ini /home/runner/work/Admini/Admini/scripts/change_database_username.php
}
...
da_swap()
{
	...
	if [ -e ${DA_DATA_USERS}/$1/reseller.conf ]; then
		generic_swap $1 $2 /home/runner/work/Admini/Admini/backend/data/admin/reseller.list
		TEMP="/usr/bin/perl -pi -e 's#reseller=${1}\$#reseller=${2}#g' /home/runner/work/Admini/Admini/backend/data/admin/ips/*";
		eval $TEMP;
		...
		if [ "$TYPE" = "admin" ]; then
			for i in `cat /home/runner/work/Admini/Admini/backend/data/admin/reseller.list; cat /home/runner/work/Admini/Admini/backend/data/admin/admin.list`; do
			{
				TEMP="/usr/bin/perl -pi -e 's#creator=${1}\$#creator=${2}#g' ${DA_DATA_USERS}/$i/user.conf";
				eval $TEMP;
			};
			...
			generic_swap $1 $2 /home/runner/work/Admini/Admini/backend/data/admin/admin.list			
		fi

		#to be safe, rewrite the whole pile with the updated creator, in case anyone is suspended.
		FULL_HTTP_REWRITE=1
	fi
	TEMP="/usr/bin/perl -pi -e 's#value=${1}\$#value=${2}#g' /home/runner/work/Admini/Admini/backend/data/admin/ips/*";
	eval $TEMP;
