...
DAPATH=/home/runner/work/Admini/Admini/backend
...
safe_set_file()
{
	...
	/home/runner/work/Admini/Admini/backend/directadmin taskq --debug 200 --run "action=set_permissions&from=$1&run_as=$2&path=$3&chown=$4${SAFE_RECURSIVE}${SAFE_CHMOD}"
}
...
set_user_perm()
{
	...
	SAC=`/home/runner/work/Admini/Admini/backend/directadmin c |grep '^secure_access_group=' | cut -d= -f2`
	if [ "${SAC}" = "" ]; then
		SAC=mail
	fi
...
user_homes()
{
	...
	if grep -q secure_access_group /home/runner/work/Admini/Admini/backend/conf/directadmin.conf; then
		echo "action=rewrite&value=secure_access_group" >> /home/runner/work/Admini/Admini/backend/data/task.queue
	fi
}
...
do_mysql()
{
	...
	#Check for old mysql datadir locations on debian in case ${MDIR} does not exist as a directory
	if [ -e /etc/debian_version ] && [ ! -d "${MDIR}" ]; then
...
set_domaindirs() {
	for user in `ls /home/runner/work/Admini/Admini/backend/data/users`; do
	{
...
set_dovecot()
{
	...
	for i in `all_users`; do
	{
		...
		if [ -s /home/runner/work/Admini/Admini/backend/data/users/${i}/domains.list ]; then
			for domain in `cat /home/runner/work/Admini/Admini/backend/data/users/${i}/domains.list`; do {
