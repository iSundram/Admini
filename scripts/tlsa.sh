...
do
	...
	if grep -m1 -q "^${DOMAIN_ESCAPED}:" /etc/virtual/domainowners; then
		...
		if ${DA_BIN} c | grep -m1 -q "^servername=${DOMAIN_ESCAPED}\$"; then
			...
			echo "Domain exists in /etc/virtual/domains, but is not set as a hostname in DirectAdmin. Unable to find 'servername=${DOMAIN}' in the output of '${DA_BIN} c'."
			#exit 1
		fi
	else
...
add_record() {
	${DA_BIN} taskq --run "action=dns&do=add&domain=${1}&type=TLSA&name=${2}&value=${3}&ttl=300&named_reload=yes"
	GENERATED=1
}
...
try_gen_tlsa() {
	if [ ! -x ${DA_BIN} ]; then
		...
		if ! ${DA_BIN} c | grep -m1 -q '^dns_tlsa=1$'; then
			echo 2
		else
			...
			if [ ! -f "${CERT}" ] && [ "$TLSATYPE" == "web" ]; then
				...
				if [ "$TLSATYPE" == "web" ] || [ "$TLSATYPE" == "all" ]; then
					for name in `echo ${WEB_RECORDS_TO_CLEANUP}`; do {
						${DA_BIN} taskq --run "action=dns&do=delete&domain=${DOMAIN}&type=TLSA&name=${name}"
					}
					done
				fi
