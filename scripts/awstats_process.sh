...
AWSTATS_MODE=`/home/runner/work/Admini/Admini/backend/directadmin c | grep '^awstats=' | cut -d= -f2`
if [ "${AWSTATS_MODE}" = "0" ] || [ "${AWSTATS_MODE}" = "" ] || [ "${AWSTATS_MODE}" -gt 2 ]; then
	echo "awstats not enabled from:";
	echo "/home/runner/work/Admini/Admini/backend/directadmin c | grep '^awstats='";
...
HTTPD=httpd
if [ "`/home/runner/work/Admini/Admini/backend/directadmin c | grep ^nginx= | cut -d= -f2`" -eq 1 ]; then
	HTTPD=nginx
fi
if [ "`/home/runner/work/Admini/Admini/backend/directadmin c | grep ^nginx_proxy= | cut -d= -f2`" -eq 1 ]; then
	HTTPD=nginx
fi
...
convert_awstast_to_user()
{
	...
	echo "action=delete&value=secure_disposal&user=${USER}&path=${STATS_DIR}.old" >> /home/runner/work/Admini/Admini/backend/data/task.queue
}
...
if [ "${ADD_CGI}" -eq 1 ]; then
	...
	if [ ! -e "${STATS_DIR}/awstats.pl" ]; then
		...
		GLOBAL_REV=$(grep "\$REVISION = " "${AS_PL}" | cut -d \' -f 2)
		...
		if [ "${CURRENT_REV}" = "" ]; then
			...
			NEED_UPDATING=1
		elif [ "${CURRENT_REV}" != "${GLOBAL_REV}" ]; then
	...
	EXECCGI=1;
	DC=/home/runner/work/Admini/Admini/backend/data/users/${USER}/domains/${TOP_DOMAIN}.conf
	if [ -s ${DC} ]; then
		C=`grep -c "^cgi=OFF" $DC`
		if [ "${C}" -gt 0 ]; then
			EXECCGI=0;
