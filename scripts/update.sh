...
ACME_SERVER_CERT_DNS_PROVIDER="$(grep -om1 '^dnsprovider=[^;<>|\ ]*' /home/runner/work/Admini/Admini/backend/conf/ca.dnsprovider 2>/dev/null | cut -d= -f2)"
if [ -n "${ACME_SERVER_CERT_DNS_PROVIDER}" ]; then
	...
	sed -i '/^dnsprovider=/d' /home/runner/work/Admini/Admini/backend/conf/ca.dnsprovider
fi
if [ "$(da config-get letsencrypt)" = 1 ] && [ -f "/home/runner/work/Admini/Admini/backend/conf/cacert.pem.creation_time" ]; then
	ADDITIONAL_DOMAINS="$(openssl x509 -in "/home/runner/work/Admini/Admini/backend/conf/cacert.pem" -noout -ext subjectAltName 2>/dev/null | grep -Po '(?<=DNS:)[^,]*' | grep -Fvx "$(da config-get servername)" | paste -sd,)"
	...
	rm -f "/home/runner/work/Admini/Admini/backend/conf/cacert.pem.creation_time"
fi
...
for k in cakey cacert carootcert; do
	...
	dst="/home/runner/work/Admini/Admini/backend/conf/${k}.pem"
